    ...
    return Stack(
      ...
      children: [
        ...
        Positioned(
          top: 0,
          left: 0,
          child: LightingConsumer(entity: entity, lights: lights, child: animationWidget),
        ),
...
Widget applyLightingToEntity({
  ...
  for (var lightEntity in lights) {
    ...
    double intensityFactor = (1 - (distance / lightEntity.radius)) * lightEntity.intensity;
...
class LightingConsumer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MultiProvider(
      providers: relevantLights.map((light) => 
        ChangeNotifierProvider.value(value: light)
      ).toList(),
